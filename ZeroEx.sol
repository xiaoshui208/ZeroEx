...
contract IExchangeCore {
    ...
    /// @dev 吃输入的订单.
    /// @param 包含所有订单信息的Order结构体.
    /// @param 期望吃掉taker资产的数量.
    /// @param 证明订单是maker生成的签名信息.
    /// @return 吃单数量和由maker,taker支付的手续费.
    function fillOrder(
    ...
    /// @dev 获取订单信息：状态，哈希，已经被吃的数量.
    /// @param 获取信息的订单.
    /// @return 关于订单和它的状态的订单信息，请通过LibOrder.OrderInfo 查看完整信息
    ...
    IExchangeCore
{
    ...
    /// @dev 为吃单验证上下文.要么成功要么抛出异常.
    /// @param 要被吃的订单.
    /// @param 包含订单状态，订单哈希，已经吃掉的数量.
    /// @param 订单中的taker地址.
    /// @param 用于证明订单是maker创建的签名.
    ...
    /// @param 要被吃的订单.
    /// @param 包含订单状态，订单哈希，已经吃掉的数量.
    /// @param takerAssetFillAmount taker期望吃单的数量.
    /// @param takerAssetFilledAmount 将要被吃的takerAsset数量.
    /// @param makerAssetFilledAmount 将要被转账的makerAsset数量.
    ...
    /// @dev 为取消订单验证上下文。要么成功，要么抛出异常.
    /// @param 要被取消的订单.
    /// @param 订单信息的订单状态，订单哈希，已经被吃的数量.
    ...
    ISignatureValidator
{
    ...
    /// @dev 通过钱包合约中定义的逻辑来验证签名.
    /// @param hash 任何32个字节的哈希.
    /// @param walletAddress 定义了签名验证方法的钱包合约地址
    /// @param signature 用于验证哈希被签名者签名过的签名.
    /// @return True 如果在钱包合约中提供的签名信息的签名者地址和钱包合约中配置的签名者地址一致时，返回True.
    ...
    /// @dev 通过验证者合约定义的逻辑验证签名.
    /// @param validatorAddress 验证者合约的地址.
    /// @param hash 任何32位字节哈希.
    /// @param signerAddress 签名了给定哈希的签名者地址.
    /// @param signature 用于证明哈希被签名者签名了的签名证据.
    /// @return 如果提供的签名信息的签名者地址和验证合约中配置的签名者地址一致，返回True.
    ...
    MTransactions
{
    ...
    /// @dev 调用后，订单就不能再被交易.
    ///      如果订单无效，或者发送方没有权限取消，抛出异常
    /// @param 要取消的订单，订单状态必须是 OrderStatus.FILLABLE.
        ...
        returns (OrderInfo memory orderInfo)
    {
        ...
        // 如果订单的 makerAssetAmount==0, 我们会拒绝这个订单.
        // 当交易合约正确的处理他们，他们在支持的基础设施创建边缘例子因为他们有一个极大的价格 While the Exchange contract handles them correctly, they create
        ...
        // 如果订单的 takerAssetAmount==0, 订单会被一直认为是可吃单的状态，
        // 因为0 == takerAssetAmount == orderTakerAssetFilledAmount. 
        // 为了便于区分，我们选择不支持 takerAssetAmount==0
        ...
        view
    {
        ...
        // 确保taker不支付超过期望的数量
        // NOTE: 这个声明不应该失败，这里做为一个额外的预防潜在bug的防护 
        require(
        ...
        // i.e. maker为支付takerAsset的makerAsset数量.
        // maker保证获取这个价格或者一个更好（更低）的。maker在这个吃单中的这个实际价格是:
        //    fill price = makerAssetFilledAmount / takerAssetFilledAmount
        // 我们需要 `fill price <= order price` 为对maker来说这个吃单是公平的.
        // 这个数量为:
    ...
    /// @dev 通过转移交易双方资产来撮合一个订单.
    /// @param order 包含订单信息的订单结构体.
    /// @param takerAddress 卖takerAsset和买makerAsset的taker地址.
    /// @param fillResults 吃单的数量和maker,taker支付的费用.
    ...
    MTransactions
{
    ...
    /// @dev 当前方法将在这个地址（0x交易签名方 或者 消息发送方）的上下文中调用.
    ///      如果调用一个吃单方法，该地址将替代taker.
    ///      如果调用一个取消方法，改地址将替代maker.
    /// @return 如果进入点是`executeTransaction`，返回0x交易的签名方.
    ///         如果进入点是其它方法，返回消息发送方.
    function getCurrentContextAddress()
        internal
        view
